...
if ($blnSubmitted) {
	...
	if ($theForm->isValid()) {
		...
		if (isset($_POST['p8'])) { $ccList .= "lpinfo2@artegan.com "; }
		$ccList = str_replace(" ", ", ", trim($ccList));

		$headers = array('To' => $ccList, 'From' => EMAIL_FROM, 'Cc' => "shaines@artegan.com, rcook@artegan.com, amylehrman@gmail.com ", 'Subject' => EMAIL_SUBJECT, 'X-Mailer' => 'PHP 4.x');
		$msgBody = '';
		...
		if (strcasecmp(SERVER_TYPE,'smtp') == 0) {
			...
			$mailParams['username'] = "scs\lpwebsite";
			$mailParams['password'] = "06132012Art";
